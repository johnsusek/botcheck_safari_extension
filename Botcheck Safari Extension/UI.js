...
function processTweetEl(tweetEl) {
  ...
  tweetEl
    .querySelector('.ProfileTweet-actionList')
    .insertAdjacentHTML('beforeend', markup.actionItem());
}
...
function attachEventListeners() {
  ...
  document.body.addEventListener('click', (ev) => {
    if (ev.srcElement.closest('.ProfileTweet-action--botcheck')) {
      ...
      const profile =
        ev.srcElement.closest('.ProfileHeaderCard') || ev.srcElement.closest('.ProfileCard');
...
function showThanks() {
  modalEl.querySelector('.modal-content').innerHTML = markup.modalContent({
    ...
    body:
      'Our model currently has ~90% accuracy and does make mistakes. Thank you for your response. :)',
    buttons: markup.modalButtons.thanks
  });
}
